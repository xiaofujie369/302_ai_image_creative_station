...
export async function POST(request: Request) {
  try {
    ...
    try {
      const images = await openaiImageService.edit({ prompt, image, model, size, quality, count });
      ...
      const updated = await prisma.$transaction(async (tx) => {
        await debitCredits(tx, {
          userId: user.id,
          amount: cost,
          description: `Image edit using ${model}`,
        });
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "UNSUPPORTED_IMAGE_TYPE" || error.message === "IMAGE_TOO_LARGE") {
        return Response.json({ error: error.message }, { status: 400 });
      }
      if (error.message === "INVALID_IMAGE_SIGNATURE") {
        return Response.json({ error: "Uploaded file content does not match its image type" }, { status: 400 });
      }
    }
